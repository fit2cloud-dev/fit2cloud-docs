---
title: 1Panel SSL 证书管理概述
description: 介绍 1Panel SSL 证书管理能力，以及申请、上传、自签、续签、ACME 账户和 DNS 账户等操作入口。
keywords: [1Panel SSL 证书, HTTPS 证书, ACME 证书, 证书续签, DNS 验证, 自签证书]
---

# 证书概述

**功能说明**：证书页面用于申请、上传、续签、下载和删除 SSL 证书，并管理 ACME 账户、DNS 账户和自签证书颁发机构。社区版、专业版和企业版均可使用；企业版用户需要证书查看或管理权限。

列表展示主域名、其他域名、申请方式、ACME 账户、到期时间、状态和自动续签状态，并提供详情、申请/续签、更新、编辑、同步、下载和删除等操作。实际按钮取决于证书来源和当前状态。

![证书列表](/img/1panel/websites/certificate_list.png)
<div style={{textAlign:"center",color:"#8a8f99",fontSize:"13px",margin:"6px 0 20px"}}>图 1 证书概述——应用卡片</div>

## 相关操作

- [申请证书](././certificate-create)
- [上传证书](././certificate-upload)
- [自签证书](././certificate-self-sign)
- [续签证书](././certificate-renew)
- [ACME 账户](././certificate-acme)
- [DNS 账户](././certificate-dns)

:::caution[删除和下载]
    删除证书前应确认没有网站仍在使用。下载包中可能包含私钥，应通过安全渠道保存和传输。

:::