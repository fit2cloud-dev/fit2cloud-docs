...
**功能说明**：GPU 监控用于查看当前节点的显卡信息、实时指标、占用进程和历史趋势。入口为左侧菜单 **AI -> GPU 监控**，页面包含 **当前状态** 和 **历史记录**。
...
![GPU 当前状态](/img/1panel/ai/gpu_monitor.png)
<div style={{textAlign:"center",color:"#8a8f99",fontSize:"13px",margin:"6px 0 20px"}}>图 1 当前状态界面</div>
...
![GPU历史数据记录](/img/1panel/ai/gpu_history1.png)
<div style={{textAlign:"center",color:"#8a8f99",fontSize:"13px",margin:"6px 0 20px"}}>图 2 历史记录界面</div>
![GPU历史数据记录](/img/1panel/ai/gpu_history2.png)
<div style={{textAlign:"center",color:"#8a8f99",fontSize:"13px",margin:"6px 0 20px"}}>图 3 历史记录界面（续）</div>

## 4 在容器中使用 GPU

驱动和容器工具配置完成后，可在支持 GPU 的应用或容器配置中启用 **GPU 加速**，并选择需要分配的设备。容器启动后，可通过容器日志、主机 GPU 进程和利用率确认设备是否生效。

![应用 GPU 加速](/img/1panel/ai/gpu_acceleration.png)
<div style={{textAlign:"center",color:"#8a8f99",fontSize:"13px",margin:"6px 0 20px"}}>图 4 在容器中使用 GPU界面</div>
