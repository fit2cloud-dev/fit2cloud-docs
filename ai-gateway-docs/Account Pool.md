...
![账号池列表](ai-gateway-docs/static/img/Account Pool/image1_account_pool_list.png)

<center>图 1：账号池列表</center>
...
![供应商下拉](ai-gateway-docs/static/img/Account Pool/image2_provider_dropdown.png)

<center>图 2：供应商下拉</center>
...
![添加账号表单](ai-gateway-docs/static/img/Account Pool/image3_add_account_form.png)

<center>图 3：添加账号表单</center>
...
> **注意**：编辑已有账号时，账号类型和协议类型不可直接修改；如需变更，通常需要重新创建账号。
...
![DeepSeek 账号表单](ai-gateway-docs/static/img/Account Pool/image4_deepseek_form.png)

<center>图 4：DeepSeek 账号表单</center>

## 3.4 核对支持协议与认证方式

系统会根据所选供应商自动列出支持协议。以自定义供应商为例，可选择多个协议，并为每个协议配置路由与认证方式。支持的认证方式包括 Bearer Token、x-api-key 或 x-goog-api-key。

![自定义供应商表单](ai-gateway-docs/static/img/Account Pool/image5_custom_provider_form.png)

<center>图 5：自定义供应商表单</center>
...
![模型映射](ai-gateway-docs/static/img/Account Pool/image6_model_mapping.png)

<center>图 6：模型映射</center>
